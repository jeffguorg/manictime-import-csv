...
def insert(**kwargs):
    import requests
    response = requests.post("{scheme}://{host}/Users/Create".format(**kwargs), cookies=kwargs['cookies'], data={
        "name": kwargs['name'],
        "roleId": 4,
...
def main(args):
    ...
    for listfile in args.lists:
        ...
        for name, email in userlist:
            ...
            if exists(username=email, scheme=args.scheme, host=args.host, cookies=cookies):
                logging.warning("User with email {} exists".format(email))
            else:
                insert(name=name, username=email, scheme=args.scheme, host=args.host, cookies=cookies)
